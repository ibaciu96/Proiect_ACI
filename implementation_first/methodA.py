...
from implementation_first.method import do_show_image, Method, MODE, ELLIPSE, RECTANGLE, POLYGON
from implementation_first.shape import random_shape, Shape
...
class MethodA(Method):
    def __fitness__(self, individual: Shape) -> float:
        ...
        try:
            fitness = np.sqrt(np.array(diff_image).sum(axis=0)).sum() * individual.z_index
    ...
    def run(self, fitness_option: str, keep_best: float = .1, changes: int = 2,
        ...
        for generation in range(1, self.generations_no + 1):
            ...
            # generation += 1
            # if abs(self.population[0].fitness - self.population[1].fitness) >= .00001: shouldNotEnd = False
            ...
            if generation % 100 == 0:
                ...
                print("Thread - {:2} -> Generation: {:10}| Best: {:10.10f}| Worst: {:10.10f}".format(th_number, generation, best_fitness, worst_fitness))
