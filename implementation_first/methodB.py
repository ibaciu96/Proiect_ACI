from implementation_first.image import ShapesImage
from implementation_first.method import do_show_image, Method, MODE, ELLIPSE, RECTANGLE, POLYGON, PIXEL_TH
...
class MethodB(Method):
    def __fitness__(self, individual: ShapesImage) -> float:
        ...
        diff_image = ImageChops.difference(self.original_image, bkg).convert('RGB')
        try:
            fitness = np.sqrt(np.array(diff_image).sum(axis=0)).sum()
            individual.fitness = 1 / fitness if fitness > 0 else 0
            return fitness
    ...
    def __crossover__(self, individual1: ShapesImage, individual2: ShapesImage, no_attributes_to_change: int = 2) -> (ShapesImage, ShapesImage):
        ...
        :param individual1: implementation_first image
        :param individual2: implementation_second image
    ...
    def run(self, fitness_option: str, keep_best: float = .2, changes: int = 2,
        ...
        # Generate implementation_first generation
        ...
        # Print a random individual in the implementation_first generation
